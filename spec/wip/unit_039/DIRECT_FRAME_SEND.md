...
| 実機操作者 | マウスでカメラを操作する | 低速・中速・高速操作、停止、切断で不自然な残留回転がない | Switch実機確認を完了条件に含める |
...
|---|---|---|---|---|
| todo | runtime dependencyがswbt-python 0.4系で、Direct公開型をroot importできる | new | package | lockとwheel metadataを確認する |
| todo | 通常adapter factoryがDirectProControllerを生成し、report periodを渡さない | new | unit | 周期送信constructor経路を削除する |
| todo | InputPublisherが評価に使った時間を非負のsample durationとしてframeへ記録する | new | unit | catch-up時の設定周期を含む |
| todo | 集約境界が最新の持続状態と3軸角変位を保持し、単一frameでは元のrateを再現する | new | unit | display latestとpendingを分離する |
| todo | 複数評価を集約しても軸ごとの角変位が保存される | new | unit | 不規則間隔と方向反転を含む |
| todo | 同一集約窓のbutton edgeは保持せず、最新stateを送信する | new | unit | 承認済みの対象外挙動 |
| todo | stale sequenceを拒否し、新epochと安全境界がpendingを破棄して理由を記録する | edge | unit | in-flight sendは再送しない |
| todo | 集約量が増えても定数サイズを保ち、診断最大値を更新する | new | unit | 上限・警告・切断なし |
| todo | ControllerFrame1件が完全InputState1件へ変換され、send完了までadapter呼び出しが完了しない | new | unit | fake Direct gamepadを使う |
| todo | 接続初期restの成功後だけCONNECTEDとなり、send失敗ではREADYへ回復する | regression | integration | 0G診断例外を維持する |
| todo | send中の複数評価が次の1送信へ集約され、送信列に重複や暗黙再送がない | new | integration | blockable fake adapterを使う |
| todo | send失敗後は後続frameを送らず、既存の安全な回復を行う | regression | integration | error分類を確認する |
| todo | watchdog、disconnect、shutdown、色再生成でrestとclose neutralを重複送信しない | regression | integration | rest失敗時だけneutral=true |
| todo | Direct移行後もworker shutdownが5秒以内に終了し、送信taskを残さない | regression | integration | active send取消を含む |
| todo | Direct送信の操作感と安全操作をSwitch実機で記録する | new | hardware | 実施不可ならunitを完了移動しない |
...
|---|---|---|
| `uv run pytest tests/unit/controller/test_swbt_dependency.py -q` | not run | package boundary item後に実行 |
| `uv run pytest tests/unit/controller tests/unit/input/test_publisher.py -q` | not run | unit TDD itemごとに実行 |
| `uv run pytest tests/integration/controller -q` | not run | runtime/lifecycle item後に実行 |
| `uv lock --check` | not run | package metadata変更後に実行 |
| `uv run ruff format --check .` | not run | final gate |
| `uv run ruff check .` | not run | final gate |
| `uv run ty check --no-progress` | not run | public boundary変更後に実行 |
| `uv run pytest tests/unit` | not run | final gate |
| `uv run pytest tests/integration` | not run | final gate |
| `uv build` | not run | package metadata変更後に実行 |
| `git diff --check` | not run | final gate |

## 10. 先送り事項

- Switch側で評価時間を明示的に表せる公開swbt APIはない。標準IMU reportでの総回転量は実機試験で確認し、問題があれば別Issueとしてswbt-python APIの拡張から検討する。

## 11. チェックリスト

- [x] 対象範囲と対象外を確認した
- [x] TDD Test List を作成した
- [ ] 全TDD itemをgreenまたは明示的にdeferredへ更新した
- [ ] 初期仕様とpublic API docstringを更新した
- [ ] package / release / public API のgateを記録した
- [ ] 専用USB BluetoothドングルとSwitch実機の結果を記録した
- [ ] 検証結果または未実行理由を記録した
