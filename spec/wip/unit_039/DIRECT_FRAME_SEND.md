...
| in progress | Direct送信の操作感と安全操作をSwitch実機で記録する | new | hardware | pairing、接続、切断は通過。ポインター未捕捉時のキーボード入力とジャイロの滑らかさは失敗し、原因を調査中 |
...
| `uv run pytest tests/unit -q --basetemp=tmp/pytest-unit039` | passed | 293 passed |
| `uv run pytest tests/integration -q --basetemp=tmp/pytest-integration039b` | passed | 120 passed |
| `uv build` | passed | sdistとwheelを生成 |
| `git diff --check` | passed | final gate |
| Switch 2実機試験 | partial failure | pairing、接続、切断は通過。ポインター未捕捉時のF→AはSwitchで認識されず、ジャイロにカクつきを観測。詳細は`spec/hardware-test-log.md` |

## 10. 先送り事項

- 観測: Switch 2実機で、ポインター未捕捉時はGUIプレビューでAが押下表示になってもF→Aが認識されない。`Start mouse`後は認識される。`pointer_capture_active`はswbt変換対象ではないため、原因はマウス由来IMU値または送信タイミングにある可能性がある。原因は未確定。
- 観測: ジャイロにカクつきがある。送信完了が評価周期を超えるとProject_Demiは複数frameを1件へ集約する。標準IMU reportで評価時間を表す公開swbt APIはないため、集約とSwitch側の解釈の関係は未検証。
- 後続: `ac37dfc`で追加したDEBUGログから、ボタン状態の送信完了と集約区間を採取して原因を分ける。必要ならswbt-python API拡張を別Issueとして検討する。
