...
|---|---|---|---|---|
| todo | swbt-python v0.3 系 dependency と公開 import 境界が成立する | new / characterization | package | pyproject、uv.lock、`swbt` import を確認 |
| todo | 全 LogicalButton、normalized stick、rad/s gyro、G accel が public swbt values へ変換される | new / edge | unit | raw 換算を行わず、実物の swbt value validation を使う |
| todo | neutral が 3 IMU slots の gyro 0 / accel +1G として 1 InputState へ変換される | new / regression | unit | `InputState.neutral()` の 0G を使わないことを固定 |
...
| todo | StartPairing の bond path 境界が runtime command と fake adapter に反映される | new / regression | unit / integration | Unit 005 contract の Intent Delta を回帰固定 |
