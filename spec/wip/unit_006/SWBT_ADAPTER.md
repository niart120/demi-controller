...
| `uv run pytest tests/unit/controller/test_swbt_adapter.py -q` | passed | 3 passed。AdapterInfo descriptor と ControllerColors / bond path 境界を確認 |
...
| `uv run pytest tests/integration/controller/test_swbt_lifecycle.py -q` | passed | 1 passed。saved reconnect と explicit pairing の lifecycle を確認 |
| `uv run pytest tests/integration/controller/test_swbt_lifecycle.py -q` | passed | 2 passed。frame apply、色再生成、saved bond / timeout 維持、close を確認 |
