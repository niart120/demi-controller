...
| red | swbt の 3 IMU slot が移動中の連続した標本として変換される | characterization / regression | unit | 非 0 report は `[-54, -54, -54]`、次の 0 report は `[0, 0, 0]` となる |
...
Project_Demi の非 0、0 を交互にした `ControllerFrame` を swbt-python 0.3.0 の標準 0x30 report へ変換し、各 report の raw gyro Z を読み戻した。
...
swbt-python は Project_Demi の current state を欠落させず、同一角速度を 3 slot へ格納する。仮説 4 は断続を新たに発生させる原因としては棄却する。一方で、入力側のパルスを 3 個の非 0 標本と 3 個の 0 標本の塊として host へ伝えるため、断続を report 内で保持する伝播要因である。修正位置は swbt アダプターより前のマウス角速度生成境界とする。
...
| `uv run python -` による 0x30 report の 3 IMU slot 読み戻し | passed | 非 0 report は raw gyro Z `[-54, -54, -54]`、0 report は `[0, 0, 0]` となり、入力パルスを report 単位で保持した |
