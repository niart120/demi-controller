...
| todo | 機材不要の標準 gate と package build が通る | characterization | package | 方向反転修正と接続先モック追加後に再実行する |
...
| `spec/wip/unit_024/SMOOTH_MOUSE_GYRO.md` | new | 原因分析、TDD 状態、検証結果、機材不要受入と実機受入の分離 |
...
| `uv run pytest tests/unit -q -p no:cacheprovider` | passed (207 passed) | 全 unit tree |
