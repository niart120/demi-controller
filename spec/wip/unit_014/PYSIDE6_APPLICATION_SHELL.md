...
| initial design | milestone 0 で更新する application / lifecycle / testing 契約。仕様執筆時点では未更新 | `spec/initial/architecture.md`, `spec/initial/lifecycle.md`, `spec/initial/testing.md` |
...
| prerequisite | pyglet と旧 UI の production / package 境界を撤去した状態 | `spec/wip/unit_013/LEGACY_UI_AND_PYGLET_REMOVAL.md` |

milestone 0 の初期仕様更新と unit_013 の撤去完了を着手条件とする。本 unit は PySide6 shell だけを所有し、入力・標準 control・production runtime 統合を先行実装しない。

仕様執筆時点では milestone 0 と unit_013 の実装は未完了である。着手時に両方の完了記録を確認し、更新後の初期仕様と unit_013 の引き渡し条件を入力にする。
...
- `spec/wip/unit_013/LEGACY_UI_AND_PYGLET_REMOVAL.md`
- `AGENTS.md`
...
|---|---|---|---|---|
| todo | 引数なしCLIはPySide6 application runnerを一度だけ呼び、Qt event loopの終了statusを返す | regression | unit | 3つのentry pointで共通runnerを使う |
...
| todo | runnerは既存instanceを尊重し、同じprocessで`QApplication`を二重生成しない | edge | unit | test fixtureとの共存を含む |
| todo | main windowは既定960x640、最小800x520、resize可能なshellとして開始する | regression | unit | offscreen widget stateを観測する |
| todo | 保存済みwidth / height / maximizedはmain windowへ適用され、close時に有効なstateを保存する | regression | integration | maximized時のnormal寸法を失わない |
| todo | closeEventとCtrl+Qはneutral、runtime停止、settings保存、window終了を一度だけ要求する | regression | integration | fake runtime / repositoryで順序と冪等性を観測する |
...
| `uv sync --dev` | not run | 仕様執筆だけでdependencyを変更していない。実装時にPySide6 wheel解決を確認する |
| `uv lock --check` | not run | 仕様執筆だけでlockを変更していない。実装時は必須 |
...
- [ ] milestone 0とunit_013の前提を確認した
- [ ] PySide6 6.xの版範囲を根拠とともに固定した
