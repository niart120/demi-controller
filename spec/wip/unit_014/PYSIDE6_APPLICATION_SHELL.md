...
| `uv run ruff format --check .` | not run | unit完了時の標準gateで再実行する |
| `uv run ruff check .` | not run | unit完了時の標準gateで再実行する |
| `uv run ty check --no-progress` | passed | current source treeの型検査を実行した |
| `uv run pytest tests/unit` | not run | unit完了時の標準gateで再実行する |
| `uv run pytest tests/integration` | not run | unit完了時の標準gateで再実行する |
| `uv build` | not run | PySide6 metadataを含むpackage gateはunit完了時に実行する |
