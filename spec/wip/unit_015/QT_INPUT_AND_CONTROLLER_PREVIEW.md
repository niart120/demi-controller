...
| prerequisite | PySide6 application shellとoffscreen fixture | `spec/wip/unit_014/PYSIDE6_APPLICATION_SHELL.md` |
...
- `spec/wip/unit_014/PYSIDE6_APPLICATION_SHELL.md`
- `AGENTS.md`
...
|---|---|---|---|---|
| todo | Qt keyとmouse press / releaseは正規化された保持状態を冪等に更新し、capture外ではcontroller mappingを変更しない | regression | unit | auto-repeat、未保持release、extra buttonを含む |
| todo | F12、focus loss、dialog open、shutdownはpointer captureを解除し、input / delta /姿勢をclearしてneutral frameを発行する | regression | integration | 自動再captureなし |
| todo | framework非依存`PointerCapturePort`でapplication試験を実行でき、application / domainがPySide6をimportしない | new | unit | 旧pyglet命名を残さない |
| todo | native event filterは対象外messageを消費せず、Qtの通常処理へ返す | new | unit | Windows message fixture |
| todo | capture開始はmain windowをRaw Input mouseの唯一のforeground受信先として登録し、終了時に解除する | new | unit | Win32 API fakeでflagsとhandleを確認 |
| todo | relative `WM_INPUT` deltaは1評価周期内で加算され1回だけ消費される | new | unit | 1000 Hz相当の複数messageを注入する |
| todo | absolute input、capture外、古いepoch、対象外windowのnative eventはframeへ反映されない | edge | integration | stale input防止 |
...
| `uv run ty check --no-progress` | not run | Qt / ctypes / Protocol境界は未実装 |
...
- [ ] unit_014のapplication shell前提を確認した
- [ ] Qt key / mouse / focus adapterを実装した
- [ ] framework非依存pointer capture portへ変更した
