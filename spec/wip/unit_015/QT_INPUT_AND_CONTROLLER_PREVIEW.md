...
自動検証済みの実装状態は本書の TDD Test List と検証欄に記録する。Windows 実display / 1000 Hz mouse の手動受入は未完了であり、結果を記録するまで本 unit を完了扱いにしない。
...
| refactor-skipped | Qtが渡す`VoidPtr`のnative message addressを読み、型不一致でRaw Input captureを解除しない | regression | unit | `CtypesNativeMessageReader`は`int()`でaddressを取り出してからWin32 `MSG`を読む。PySide6型スタブが`VoidPtr.__int__()`を表現しないため、回帰試験だけで`SupportsInt`へのcastを明記する |
...
| refactor-skipped | Raw Input登録失敗はcaptureを開始せず、連続read失敗はcapture解除と安全な警告を返す | edge | integration | `RelativePointerCapturePort`はregistrationをcapture開始より先に行い、失敗時はcleanupして`IDLE`を維持する。backendは既定3回の連続read失敗でcallbackを1回だけ呼び、`ApplicationSession`は固定の安全なwarningへ変換する |
| refactor-skipped | fallback backendは補正後値と利用不能を区別し、未確認値を`RAW_UNACCELERATED`と報告しない | new | unit | `QtRelativePointerBackend`はQt型を持たず、位置差分をcurrent epochだけに限定して`RELATIVE_ACCELERATED`で渡す。取得できないplatform pathは`UNAVAILABLE`となり、fallback自身はrawを報告しない |
| refactor-skipped | Raw Input登録中もQtのbutton、focus、dialog用mouse eventが通常どおり届く | regression | integration | `MainWindow`がapplication-wide Qt event filterとnative filterを所有する。native filterはQt eventを消費せず、子widgetの`FocusOut`はcapture解除とせず、main windowのdeactivateとdialog開始だけをcoordinatorへ渡す |
| refactor-skipped | 8ミリ秒評価で生成した同一`ControllerFrame`をruntime portとpreviewが観測する | regression | integration | `ControllerFrameFanout`がruntimeと`FramePreviewPort`へ同一immutable frameを渡す。`MainWindow`は`PreciseTimer`の8ms timeoutから`CaptureCoordinator.evaluate()`だけを呼び、preview用の再評価を作らない |
| refactor-skipped | previewはbutton、stick、gyro、accel、capture、colorを1つのframe modelから更新する | regression | unit | `ControllerPreviewModel`は完全な`ControllerFrame`と`ControllerColorSettings`だけから作る。`ControllerPreviewWidget`はmodelを保存して`QPainter`で描画し、`MainWindow.set_frame()`はfan-outされたframeをそのままwidgetへ渡す |
| refactor-skipped | frameが8ミリ秒で更新されてもpreviewの再描画要求は最大60Hzとなり、常に最新frameを使う | new | unit | `PreviewRepaintLimiter`は単調時計でframe由来の`update()` requestだけを60Hz以下に制限する。modelは毎frame更新するため、次の許可されたrepaintは常に最新frameを描画する。同期`repaint()`は使わない |
| passed | Windows実displayで画面端、1000 Hz mouse、通常Qt操作、focus loss、F12解除を確認する | new | manual | 環境値と観測結果を記録した。packet / frameの正確な数値と開始 / 終了時刻は未記録 |
...
| `tools/manual_ui_acceptance.py` | new | production境界を使うWindows手動受入ランチャー |
...
| `uv run pytest tests/unit/platform/test_windows_raw_input.py -q -p no:cacheprovider` / `uv run ruff format --check src/demi/platform/windows_raw_input.py tests/unit/platform/test_windows_raw_input.py` / `uv run ruff check src/demi/platform/windows_raw_input.py tests/unit/platform/test_windows_raw_input.py` / `uv run ty check --no-progress` / `git diff --check` | passed (4 passed) | PySide6がnative filterへ渡す`shiboken6.Shiboken.VoidPtr`をWin32 `MSG` addressとして変換し、captureを`relative_pointer_read`で解除しない回帰を確認した |
...
| `uv run pytest tests/integration/ui/test_windows_raw_input_capture.py -q -p no:cacheprovider` | expected failed (2 failed) | red: coordinatorにrelative pointer lifecycle、backendに連続read failure callbackが存在しなかった |
| `uv run pytest tests/integration/ui/test_windows_raw_input_capture.py tests/unit/application/test_coordinator.py tests/unit/application/test_app.py -q -p no:cacheprovider` / `uv run ruff format --check src/demi/application/coordinator.py src/demi/platform/windows_raw_input.py src/demi/app.py tests/integration/ui/test_windows_raw_input_capture.py` / `uv run ruff check src/demi/application/coordinator.py src/demi/platform/windows_raw_input.py src/demi/app.py tests/integration/ui/test_windows_raw_input_capture.py` / `uv run ty check --no-progress` / `git diff --check` | passed (19 passed) | green: registration failure時のcleanupと`IDLE`維持、連続2回のread failureでのneutralize、固定warningへの変換、native handleを表示しないことを確認した |
| `uv run pytest tests/unit/input/test_relative_pointer.py -q -p no:cacheprovider` | expected failed (collection error) | red: `RelativePointerBackend`とfallback capability契約が未実装だった |
| `uv run pytest tests/unit/input/test_relative_pointer.py -q -p no:cacheprovider` / `uv run ruff format --check src/demi/input/relative_pointer.py tests/unit/input/test_relative_pointer.py` / `uv run ruff check src/demi/input/relative_pointer.py tests/unit/input/test_relative_pointer.py` / `uv run ty check --no-progress` / `git diff --check` | passed (3 passed) | green: Qt fallbackが`RELATIVE_ACCELERATED`または`UNAVAILABLE`だけを返し、基準位置とcurrent epochからdeltaを正規化することを確認した |
| `uv run pytest tests/integration/ui/test_qt_input_capture.py -q -p no:cacheprovider` | expected failed (1 failed) | red: `MainWindow.configure_input`がなく、Qt application filterとnative filterを所有できなかった |
| `uv run pytest tests/integration/ui/test_qt_input_capture.py tests/integration/ui/test_application_lifecycle.py tests/unit/application/test_app.py tests/unit/input/test_qt_adapter.py tests/unit/platform/test_windows_raw_input.py tests/unit/ui/test_application.py -q -p no:cacheprovider` / `uv run ruff format --check src/demi/application/coordinator.py src/demi/input/qt_adapter.py src/demi/platform/windows_raw_input.py src/demi/ui/main_window.py src/demi/app.py tests/integration/ui/test_qt_input_capture.py tests/unit/application/test_app.py` / `uv run ruff check src/demi/application/coordinator.py src/demi/input/qt_adapter.py src/demi/platform/windows_raw_input.py src/demi/ui/main_window.py src/demi/app.py tests/integration/ui/test_qt_input_capture.py tests/unit/application/test_app.py` / `uv run ty check --no-progress` / `git diff --check` | passed (28 passed) | green: Raw Input登録中のbutton click、子widgetのfocus、main window deactivate、dialog開始、composition rootからのinput setupを確認した |
| `uv run pytest tests/integration/ui/test_input_evaluation.py -q -p no:cacheprovider` | expected failed (collection error) | red: runtimeとpreviewへ同一frameを渡すfan-out境界が未実装だった |
| `uv run pytest tests/integration/ui/test_input_evaluation.py tests/integration/ui/test_qt_input_capture.py tests/integration/ui/test_application_lifecycle.py tests/unit/application/test_app.py tests/unit/input/test_publisher.py tests/unit/ui/test_application.py -q -p no:cacheprovider` / `uv run ruff format --check src/demi/application/frame_fanout.py src/demi/ui/main_window.py src/demi/app.py tests/integration/ui/test_input_evaluation.py` / `uv run ruff check src/demi/application/frame_fanout.py src/demi/ui/main_window.py src/demi/app.py tests/integration/ui/test_input_evaluation.py` / `uv run ty check --no-progress` / `git diff --check` | passed (30 passed) | green: 8msの`PreciseTimer` timeoutから評価し、runtimeとmain windowが同一`ControllerFrame` objectを観測することを確認した |
| `uv run pytest tests/unit/ui/test_controller_preview.py -q -p no:cacheprovider` | expected failed (collection error) | red: preview modelと`QPainter` widgetが未実装だった |
| `uv run pytest tests/unit/ui/test_controller_preview.py tests/unit/ui/test_application.py tests/unit/application/test_app.py tests/integration/ui/test_input_evaluation.py tests/integration/ui/test_qt_input_capture.py tests/integration/ui/test_application_lifecycle.py -q -p no:cacheprovider` / `uv run ruff format --check src/demi/application/frame_fanout.py src/demi/app.py src/demi/ui/controller_preview.py src/demi/ui/main_window.py tests/unit/application/test_app.py tests/unit/ui/test_controller_preview.py` / `uv run ruff check src/demi/application/frame_fanout.py src/demi/app.py src/demi/ui/controller_preview.py src/demi/ui/main_window.py tests/unit/application/test_app.py tests/unit/ui/test_controller_preview.py` / `uv run ty check --no-progress` / `git diff --check` | passed (27 passed) | green: 四つの色、button、stick、gyro、accel、captureをmodelへ反映し、offscreen `QPainter`描画、main window forwarding、保存済み色のcompositionを確認した |
| `uv run pytest tests/unit/ui/test_controller_preview.py::test_preview_limits_repaint_requests_to_sixty_hz_but_keeps_the_latest_frame -q -p no:cacheprovider` | expected failed (1 failed) | red: previewへ単調時計とrepaint request境界を渡せず、60Hz制限を試験できなかった |
| `uv run pytest tests/unit/ui/test_controller_preview.py tests/unit/ui/test_application.py tests/unit/application/test_app.py tests/integration/ui/test_input_evaluation.py tests/integration/ui/test_qt_input_capture.py tests/integration/ui/test_application_lifecycle.py -q -p no:cacheprovider` / `uv run ruff format --check src/demi/ui/controller_preview.py src/demi/ui/main_window.py src/demi/application/frame_fanout.py src/demi/app.py tests/unit/ui/test_controller_preview.py tests/unit/application/test_app.py` / `uv run ruff check src/demi/ui/controller_preview.py src/demi/ui/main_window.py src/demi/application/frame_fanout.py src/demi/app.py tests/unit/ui/test_controller_preview.py tests/unit/application/test_app.py` / `uv run ty check --no-progress` / `git diff --check` | passed (28 passed) | green: fake monotonic clockで0msと17msだけrequestし、8ms更新中もmodelがsequence 4の最新frameを保持することを確認した |
| `uv sync --dev` / `uv lock --check` | passed | lock と開発依存の整合を確認した |
| `uv run ruff format --check .` | passed | 95 files already formatted |
| `uv run ruff check .` | passed | All checks passed |
| `uv run ty check --no-progress` | passed | Qt input adapter、ctypes、Protocol を含む現在の型境界を確認した |
| `uv run pytest tests/unit` | passed (179 passed) | 通常の cache 書き込みを許可した標準コマンドで確認した。sandbox 内では `.pytest_cache` への書き込みが拒否されるため、cache 無効化した同じ試験も通過した |
| `uv run pytest tests/integration` | passed (20 passed) | 通常の cache 書き込みを許可した標準コマンドで確認した。sandbox 内では `.pytest_cache` への書き込みが拒否されるため、cache 無効化した同じ試験も通過した |
| `uv build` | passed | `demi_controller-0.1.0.tar.gz` と `demi_controller-0.1.0-py3-none-any.whl` を生成した |
| `git diff --check` | passed | whitespace error なし |
| `uv sync --dev` / `uv lock --check` / `uv run ruff format --check .` / `uv run ruff check .` / `uv run ty check --no-progress` / `uv run pytest tests/unit` / `uv run pytest tests/integration` / `uv build` / `git diff --check` | passed | `VoidPtr`回帰修正後の一括gate。format 96 files、unit 180 passed、integration 20 passed。初回の`uv build`はsandboxのPyPI socket制限（os error 10013）で失敗し、同じcommandを許可済み環境で再実行してsdist / wheelを生成した |
| GitHub Actions PR #19のLinux / macOS test | expected failed | `test_ctypes_native_message_reader_accepts_qt_void_pointer`がWindows外で`RawInputUnavailableError`となった。`CtypesNativeMessageReader`はWin32 `MSG`を読むため、非Windowsでこの試験を実行することは契約外である |
| `uv run pytest tests/unit/platform/test_windows_raw_input.py -q -p no:cacheprovider` / `uv run ruff format --check tests/unit/platform/test_windows_raw_input.py` / `uv run ruff check tests/unit/platform/test_windows_raw_input.py` / `uv run ty check --no-progress` / `git diff --check` | passed | Windowsで`VoidPtr`回帰を実行し、非Windowsでは同試験をskipするようにした。Windows実行結果は4 passed。PR #19の再実行はGitHub Actionsで確認する |
| `uv run ruff format --check tools/manual_ui_acceptance.py` / `uv run ruff check tools/manual_ui_acceptance.py` / `uv run ty check --no-progress tools/manual_ui_acceptance.py` / `uv run python tools/manual_ui_acceptance.py --help` | passed | 手動受入ランチャーの静的検査と引数起動を確認した。実displayでの観測結果はまだ含まない |
| `uv run python tools/manual_ui_acceptance.py --timeout-seconds 300` とComputer UseによるWindows実display確認 | passed | 2026-07-14: 入力開始の800ms後も`captured`かつ`読出し失敗: なし`、F12で`idle`、dialogで`Qt input 123`の入力と閉じる操作後に`idle`を確認した。previewに表示欠け・重なりは見つからなかった。Computer Useの注入入力であり、物理Raw Inputの合否には使わない |
| Windows実display / 物理1000 Hz mouse受入 | passed | 2026-07-15: 1000 Hz設定の物理mouseで、画面端までの入力継続、delta表示、最大間隔93 ms、read failureなし、F12の中断、外部windowへのfocus loss後の`idle`を利用者が確認した。packet / frameの正確な数値と開始 / 終了時刻は未記録 |
| macOS / Linux pointer capability受入 | not run | 対象desktop環境が必要。未実行を支援済みと記録しない |

### Windows実display受入手順

`uv run python tools/manual_ui_acceptance.py` で、production の`MainWindow`、`CaptureCoordinator`、`InputPublisher`、`WindowsRawInputBackend`を接続した手動受入ランチャーを起動する。このランチャーのtoolbarとdialogは受入専用であり、production UIではない。

### Windows実display 実験条件

以下は受入前に取得できた環境情報と、利用者が報告した物理mouse操作の観測である。未提示の観測値は推測で補わない。

| 項目 | 記録 |
|---|---|
| 記録日時 | 2026-07-14T23:51:56+09:00 |
| commit | `99580b6 fix(input): QtのVoidPtrをRaw Inputアドレスへ変換` |
| Windows版 / build | 取得不能。実行環境が`Win32_OperatingSystem`へのアクセスを拒否した |
| display | `AW2725QF`、2560 x 1440 logical pixels、logical DPI 96.0、device pixel ratio 1.5（150%相当） |
| 接続済みmouse候補 | `HID 準拠マウス` 4件、いずれも状態`OK`。この情報だけでは機種を特定できない |
| mouse DPI | 未確認 |
| polling rate | 1000 Hz設定（利用者確認）。この受入では実測周波数を別途計測していない |
| 最大間隔 | 93 ms（利用者観測） |
| delta | 記録されていたように見えた（利用者観測）。正確な値は未記録 |
| F12 | 中断機能を確認した（利用者観測） |
| 画面端 | 画面端まで移動しても入力記録は継続した（利用者観測） |
| read failure | なし（利用者観測） |
| preview | mouse移動を止めると表示の変化も停止した（利用者観測）。移動中に更新が停止する異常は観測していない |
| 開始 / 終了時刻 | 未記録 |
| packet / frame | 数値未記録 |
| focus loss | 別windowへのclick後、Demiへ戻ると`idle`であることを確認した（利用者観測） |
| 物理操作の合否 | 合格。packet / frameの正確な数値と開始 / 終了時刻は未記録だが、受入対象の動作を確認した |

1. 起動前に Windows の版、display の解像度と拡大率、mouse の機種・DPI・polling rateを記録する。polling rateはmouse vendorの設定画面または明示されたhardware仕様で1000 Hzであることを確認する。
2. `入力開始 / 解除`を選び、status barが`状態: captured`、`Raw: raw_unaccelerated`となることを確認する。画面端まで継続移動し、`packet`と`delta`が増え、controller previewのgyro / accel表示が変化することを観測する。
3. 1000 Hz mouseで最低10秒間、画面端をまたぐ連続移動を行う。`packet`、`delta`、`frame`、最後の間隔と最大間隔を記録する。数値が停滞、read failure、またはpreview停止を観測した場合は合格にしない。
4. `F12`で`状態: idle`となり、packetとdeltaが後続frameへ反映されず、カーソルが戻ることを確認する。再captureは明示操作だけで開始する。
5. capture中に別windowへfocusを移し、戻した後に`状態: idle`となることを確認する。次に`ダイアログ確認`を開き、text fieldへの文字入力と閉じるbuttonが通常どおり動くことを確認する。

手動結果には、上記の環境値、開始・終了時刻、commit、各観測値、失敗の有無を記録する。Computer Useで確認したcapture、F12、dialog、preview表示は物理mouse受入とは別記録にする。ランチャーの表示値は受信と評価の活動を示すが、packet欠落ゼロを単独で証明するものではない。
...
| capture中の外部window clickを抑止する操作方針は未決定 | focus loss時の安全なneutralizeは確認したが、別windowをclickできることが意図しない操作を起こしうる | `spec/dev-journal.md` の2026-07-15記録。必要なら別unitへ昇格する |
...
- [x] Qt key / mouse / focus adapterを実装した
- [x] framework非依存pointer capture portへ変更した
- [x] Windows native event filterとRaw Inputを実装した
- [x] GLFWをdependencyへ追加していない
- [x] capability、neutralization、8ms評価を確認した
- [x] QPainter previewと最大60Hz更新を確認した
- [x] Windows実displayの手動結果を記録した
- [x] macOS / Linuxの未実行事項を支援済みと記録していない
- [x] TDD Test Listと検証結果を更新した
- [x] unit_016 / unit_017への引き渡し条件を満たした
