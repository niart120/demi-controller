...
| Windows実display / 1000 Hz mouse受入 | pending | Raw Input capture の起動までは確認したが、実画面操作、F12、focus loss、物理 1000 Hz mouse の観測値は未記録 |
...
手動結果には、上記の環境値、開始・終了時刻、commit、各観測値、失敗の有無を記録する。ランチャーの表示値は受信と評価の活動を示すが、packet欠落ゼロを単独で証明するものではない。
