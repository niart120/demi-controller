...
| Windows実display / 物理1000 Hz mouse受入 | pending | 画面端をまたぐ連続移動、packet / delta / frameの観測値、外部windowへのfocus lossは、実mouseと利用者の手動操作で別記録する |
...
| packet / frame | 数値未記録 |
| focus loss | 結果未記録 |
| 物理操作の合否 | 保留。TDD itemの全観測条件を満たしたか判定できる記録が不足している |
...
| Windowsの物理1000 Hz mouse、画面端、外部windowへのfocus lossは未記録 | Computer Useの注入入力では物理Raw Inputとpolling rateを判定できない | unit_015の別建て手動受入記録 |
