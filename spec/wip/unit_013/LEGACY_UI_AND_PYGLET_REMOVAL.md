...
|---|---|---|---|---|
| todo | pyglet 非導入環境で package import と `demi`、`project-demi`、`python -m demi` の version 表示が成功する | regression | package | GUI runner と display を生成しない |
| todo | 引数なし CLI は旧 UI 撤去中を安全に説明して非ゼロ status を返し、pyglet 欠落 traceback を表示しない | edge | unit | unit_014 で GUI 起動へ置換する |
| todo | production source と現行 test の pyglet import、旧 UI class、pyglet固有 type / factory が0件になる | new | package | `spec/complete` は検索対象外 |
| todo | `pyproject.toml` と `uv.lock` の dependency graph に pyglet が存在しない | regression | package | `uv lock --check` で固定する |
| todo | package builder と license inventory に pyglet の収集・列挙指定が存在しない | regression | package | PySide6 license 追加はunit_014/018で扱う |
| todo | tag または手動 package workflow が旧 standalone artifact を build / upload しない | regression | package | milestone 7 まで停止する |
