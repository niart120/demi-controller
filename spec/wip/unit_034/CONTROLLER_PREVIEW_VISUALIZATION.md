...
|---|---|---|---|---|
| todo | minimum、default、large sizeで全control boundsがcanvas内に収まり、許可していないcontrol同士が交差しない | new / regression | unit | pure layout modelを直接検証する |
| todo | neutral frameにもA/B/X/Y、D-pad、L/R/ZL/ZR、Plus/Minus/HOME/Capture、両stickとclickがすべて含まれる | regression | unit | control id集合で欠落を検出する |
| todo | 各digital inputと複数同時入力が対応controlのpressed stateだけを更新する | regression | unit | D-padの先頭文字省略を廃止する |
| todo | analog stick値は可動範囲へclampされ、stick clickはaxis位置と独立して表示される | regression | unit | edge値と範囲外値を含める |
| todo | gyroの正負3軸が互いに異なる回転方向としてmodel化され、値の大きさが表示量へ単調に反映される | new | unit | absolute poseをassertしない |
