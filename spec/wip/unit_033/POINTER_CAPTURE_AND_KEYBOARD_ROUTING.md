...
|---|---|---|---|---|
| todo | focused operational stateではpointer capture offでもkeyboard bindingを評価し、mouse sourceはneutralのままとなる | new / regression | unit | keyboard / mouse の有効条件を分離する |
| todo | pointer capture開始前後で保持keyboard入力を維持し、mouse buttonとrelative motionだけを追加評価する | new / regression | integration | epoch変更でkey releaseを失わない |
