...
|---|---|---|---|---|
| todo | `demi --version` と `python -m demi --version` が同じ版を表示する | new | package | 現在は module entry point が存在しない |
| todo | `project-demi --version` が canonical CLI と同じ結果になる | new | package | roadmap / initial naming との互換境界 |
| todo | build した配布物が metadata、import ルート、`py.typed` を保持する | characterization | package | `uv build` と package smoke を使う |
| todo | push / pull request の CI が標準 gate を実行する | new | package | YAML の対象 command と marker scope を確認する |
...
| `.github/workflows/ci.yml` | new | 最小 CI gate |
| `.github/PULL_REQUEST_TEMPLATE.md` | new | merge cleanup が要求する evidence 欄 |
| `spec/wip/unit_001/PROJECT_BOOTSTRAP.md` | new / modify | TDD 状態、検証、完了記録 |
...
| `uv build` | not run | sandbox のネットワーク制限で `uv-build` を取得できず、実装後に承認付きで再実行する |
| `uv run demi --version` | not applicable | 現行 CLI は引数を処理せず `Project_Demi` を表示するため、この unit の TDD で置き換える |
...
- [ ] package / release / public API に触れる gate を完了した
- [ ] 完了時に `spec/complete/unit_001` へ移動した
