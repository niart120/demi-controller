...
| refactor-skipped | F4またはtoolbarでpointer captureを解除するとmouse stateだけが消え、保持keyboard bindingは次frameにも残る | new / edge | integration | 193件green。pointer-only stopと全入力neutralizationを別methodに分離済みで追加の構造変更なし |
| refactor-skipped | focus loss、dialog、shutdownはpointer stateに関係なく全入力とposeをneutralizeする | regression | integration | 194件green。既存の全入力leave boundaryをidle focus lossにも適用したため追加の構造変更なし |
| refactor-skipped | built-in defaultはHOME=F1、release=F4となり、exact legacy defaultsだけを移行してcustom値を保持する | new / regression | unit | unit 256件green。schema v1の型付きdecode後補完へ限定したため追加の構造変更なし |
| refactor-skipped | F4は新規bindingを拒否し、F12は通常bindingとして保存・評価できる | regression | unit / integration | 331件green。既存reserved key境界をF4へ置換したため追加の構造変更なし |
| refactor-skipped | Qt event filterはfocused operational stateでpointer capture offでもkeyboard eventを保持し、mouse eventは無視する | regression | unit / integration | 23件green。keyboard activity callbackをpointer callbackから分離したため追加の構造変更なし |
| refactor-skipped | toolbar、status、previewはkeyboard activityではなくpointer captureのon/offを表示する | new / regression | unit / integration | unit 258件、UI 68+3件green。frameへpointer flagを分離済みで追加の構造変更なし |
| not-run | Windowsの通常描画と実mouseでF4解除後に外部windowへmouseが配送され、keyboard mappingは継続する | new | manual | 通常描画と自動統合testはpass。物理mouseを使う外部window配送は未実行 |
...
| `uv run pytest -p no:cacheprovider tests/unit/application tests/unit/input tests/integration/input tests/integration/ui -q` | pass | 193 passed。F4 / toolbar後のkeyboard継続と、focus / dialog / shutdown / watchdog / controller errorの全入力neutralizationを確認 |
| `uv run pytest -p no:cacheprovider tests/unit/application tests/unit/input tests/integration/input tests/integration/ui -q` | pass | 194 passed。pointer offのfocus lossを含む全入力・pose neutralizationを確認 |
| `uv run pytest -p no:cacheprovider tests/unit -q` | pass | 256 passed。HOME=F1、release=F4、exact legacy default移行とcustom値保持を確認 |
| `.venv\\Scripts\\pyside6-lrelease.exe src\\demi\\i18n\\demi_ja.ts -qm src\\demi\\i18n\\demi_ja.qm` | pass | 110件すべてfinished、unfinished 0件 |
| `uv run pytest -p no:cacheprovider tests/unit tests/integration/input tests/integration/ui -q` | pass | 331 passed。F4拒否、F12保存・評価、mapping dialogのF4優先解除を確認 |
| `uv run pytest -p no:cacheprovider tests/unit/input/test_qt_adapter.py tests/integration/ui/test_qt_input_capture.py tests/integration/ui/test_qt_runtime_events.py -q` | pass | 23 passed。pointer offのkeyboard event保持とmouse event無視をQt境界で確認 |
| `uv run pytest -p no:cacheprovider tests/unit -q` | pass | 258 passed。toolbar、status、preview modelがpointer capture flagを表示することを確認 |
| `uv run pytest -p no:cacheprovider tests/integration/ui --ignore=tests/integration/ui/test_source_entry_points.py -q` | pass | 68 passed |
| `uv run pytest -p no:cacheprovider tests/integration/ui/test_source_entry_points.py -q` | pass | 3 passed |
| `uv run pytest -p no:cacheprovider tests/integration/ui -q` | error | 65件通過後、Qt top-level window teardownでprocess abort。上記2processへの分割では全71件pass |
| `.venv\\Scripts\\pyside6-lrelease.exe src\\demi\\i18n\\demi_ja.ts -qm src\\demi\\i18n\\demi_ja.qm` | pass | 111件すべてfinished、unfinished 0件 |
| `uv run pytest -p no:cacheprovider --basetemp tmp/pytest/unit033-unit-rerun tests/unit -q` | pass | 258 passed |
| `$inspect-gui-states`によるpointer off / on画面確認 | pass | 通常Windows描画で`Start mouse` / `Stop mouse`と`Mouse capture: Off` / `On`を確認。fresh windowではstatus barの切れ・重なりなし。連続2枚目で一時的なlayout乱れが見えたがfresh captureでは再現せず、撮影条件由来と判断 |
| 実物理mouseによるF4解除後の外部window配送 | not run | 物理操作が必要なため自動実行していない。Windows hook / Raw Input integrationは上記68件に含めてpass |
| `uv sync --dev` | pass | 77 packagesを解決し、74 packagesを確認 |
| `uv lock --check` | pass | lock差分なし |
| `uv run ruff format --check .` | pass | 142 files already formatted |
| `uv run ruff check .` | pass | 指摘なし |
| `uv run ty check --no-progress` | pass | 型エラーなし |
| `uv run pytest -p no:cacheprovider --basetemp tmp/pytest/unit033-gate-unit tests/unit -q` | pass | 258 passed |
| `uv run pytest -p no:cacheprovider --basetemp tmp/pytest/unit033-gate-nonui tests/integration --ignore=tests/integration/ui -q` | pass | 15 passed |
| `uv run pytest -p no:cacheprovider --basetemp tmp/pytest/unit033-gate-ui tests/integration/ui --ignore=tests/integration/ui/test_source_entry_points.py -q` | pass | 68 passed |
| `uv run pytest -p no:cacheprovider --basetemp tmp/pytest/unit033-gate-entry tests/integration/ui/test_source_entry_points.py -q` | pass | 3 passed |
...
- [x] operational keyboard と pointer capture を別状態にした
- [x] HOME F1、release F4、F12許可の互換移行を確認した
- [x] Windows通常描画と自動統合testで安全解除を確認した
- [ ] Windows実物理mouseによる外部window配送を確認した（手動受入へ先送り）
