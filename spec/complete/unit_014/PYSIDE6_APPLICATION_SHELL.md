...
milestone 0とunit_013は完了済みである。着手時に両方の完了記録を確認し、更新後の初期仕様とunit_013の引き渡し条件を入力にした。
...
|---|---|---|---|---|
| refactor-skipped | 引数なしCLIはPySide6 application runnerを一度だけ呼び、Qt event loopの終了statusを返す | regression | unit | `demi.app.run_application()`を引数なし分岐で遅延importし、`GuiPort.run()`のstatusをCLIへ返す。default factoryは同じ`QtApplicationRunner`でmain windowとevent loopを所有する |
| refactor-skipped | module import、`--version`、unknown argumentは`QApplication`とmain windowを生成しない | regression | package | fresh subprocessでPySide6 importを拒否し、`import demi`、version、unknown argumentが成功することを確認した |
| refactor-skipped | 旧UI撤去の回帰testは新しい`demi.ui` packageを許可しつつ、pyglet import・backend・具象型を禁止する | regression | unit | unit_013の撤去対象はpyglet UIであり、後続unitのQt package名まで禁止しない。AST検査の対象をpyglet固有境界へ限定する |
...
| refactor-skipped | settingsなし・adapterなしでもempty main windowを表示し、ユーザーcloseでstatus 0となる | edge | integration | `QTimer`によるuser closeでevent loopを終了し、runtimeを開始せず、pairing / discoveryを送らないことをfake boundaryで確認した |
| refactor-skipped | offscreen fixtureはprocess内のapplicationを共有し、test終了後にtop-level windowを残さない | new | unit | shared `QApplication`をfixtureで返し、Qt test後にtop-level widgetをclose、deferred delete、event処理して残存をassertする |
| refactor-skipped | Windows、macOS、Linux CIでPySide6 installとdisplay-free / offscreen shell testが成功する | new | package | Python 3.12 / 3.13 matrixの`test` jobへ`QT_QPA_PLATFORM=offscreen`を設定し、既存のdependency install、unit、integration gateをQt shell testにも適用する。実行結果はPRの6 matrix jobsで確認する |
| refactor-skipped | Ubuntu CIはPySide6 importに必要なEGL runtimeを導入してからoffscreen shell testを実行する | regression | package | `libEGL.so.1`不足でUbuntu 3.12 / 3.13のtest収集が失敗したため、Linux jobだけに`libegl1`を導入する。macOS / Windows jobは変更しない |
| refactor-skipped | wheel / sdist metadataが選定したPySide6 6.xを要求し、Qt shell moduleを含む | regression | package | `uv build`後にwheelの`Requires-Dist: PySide6>=6.11,<6.12`と`demi/ui/application.py`、`demi/ui/main_window.py`、sdistの対応する`src/` pathを直接検査する。CIはtestをbuildより先に実行するため、unit testを既存`dist/`へ依存させない |
...
| `README.md` | modify | 最小Qt画面の提供範囲と単体配布物未提供を利用者向けに明記 |
...
| `tests/unit/ui/test_application.py` | new | runner、遅延import、single application、window寸法、最大化、close action |
| `tests/unit/test_cli.py` | modify | Qt runner dispatchとdisplay-free version契約 |
| `tests/integration/ui/test_application_lifecycle.py` | new | Qt shell startup / closeのfake境界 |
...
| `spec/complete/unit_014/PYSIDE6_APPLICATION_SHELL.md` | modify | TDD状態、版選定、検証、引き渡し記録 |
...
| `uv run pytest tests/unit` | expected failed (1 failed, 165 passed) | red: unit_013の回帰testが新設した`src/demi/ui`を旧pyglet UIと誤認した |
| `uv run pytest tests/unit/test_legacy_ui_removal.py -q -p no:cacheprovider` | passed (1 passed) | green: Qt package名は許可し、pyglet import、`pyglet_backend.py`、旧具象型だけを禁止する回帰範囲へ更新した |
...
| `uv run pytest tests/unit/application/test_app.py -q -p no:cacheprovider` | expected failed (1 failed, 12 passed) | red: GUI event loopのstatus 23が`run_application()`で0へ失われた |
| `uv run pytest tests/unit/application/test_app.py -q -p no:cacheprovider` | passed (13 passed) | green: `GuiPort.run()`のstatusを保持し、runtime開始・discoveryをunit_017まで送らない |
| `uv run pytest tests/unit/ui/test_application.py -q -p no:cacheprovider` | expected failed (1 failed, 4 passed) | red: default dependencyが`GuiUnavailableError`を返した |
| `uv run pytest tests/unit/ui/test_application.py -q -p no:cacheprovider` | passed (5 passed) | green: default window / GUI factoryが同じQt runnerとmain windowを共有した |
| `uv run pytest tests/unit/test_cli.py -q -p no:cacheprovider` | passed (10 passed) | characterization: fresh subprocessでPySide6 importを拒否してもpackage import、version、unknown argumentが成功した |
| `uv run pytest tests/integration/ui/test_application_lifecycle.py -q -p no:cacheprovider` | expected failed (1 failed, 1 passed) | red: empty shellがruntimeを開始し`DiscoverAdapters()`を送った |
| `uv run pytest tests/integration/ui/test_application_lifecycle.py -q -p no:cacheprovider` | passed (2 passed) | green: settingsなし・adapterなしのshellがuser closeでstatus 0となり、runtime開始とdiscoveryを行わない |
| `uv run pytest tests/unit/test_cli.py tests/unit/application/test_app.py tests/unit/ui/test_application.py -q -p no:cacheprovider` | passed (28 passed) | current CLI / application / Qt shell unit契約を確認した |
| `uv run pytest tests/integration/ui/test_application_lifecycle.py -q -p no:cacheprovider` | passed (2 passed) | current Qt lifecycle integration契約を確認した |
| `uv run ruff format --check <changed files>` / `uv run ruff check <changed files>` / `uv run ty check --no-progress` | passed | formatter、lint、型境界を確認した |
| `uv run pytest tests/unit/ui/test_application.py -q -p no:cacheprovider` | expected failed (collection error) | red: top-level window cleanup helperがfixtureに存在しなかった |
| `uv run pytest tests/unit/ui/test_application.py -q -p no:cacheprovider` | passed (6 passed) | green: 共有applicationの再利用、eager cleanup、test teardown時のwindow残存検査を確認した |
| `uv run pytest tests/unit/test_ci.py -q -p no:cacheprovider` | expected failed (1 failed, 1 passed) | red: 3 OS test jobにQt offscreen platformの設定がなかった |
| `uv run pytest tests/unit/test_ci.py -q -p no:cacheprovider` | passed (2 passed) | green: `QT_QPA_PLATFORM=offscreen`をWindows、macOS、Linuxのtest job全体へ設定するworkflow契約を確認した。remote matrixの実行結果はPRで確認する |
| PR #18 のCI | failed | macOS / Windowsの4 jobsはpassed。Ubuntu 3.12 / 3.13では`PySide6.QtGui` import時に`libEGL.so.1`不足でtest収集が停止した |
| `uv run pytest tests/unit/test_ci.py -q -p no:cacheprovider` | expected failed (1 failed, 2 passed) | red: Linux jobがPySide6用のEGL runtimeを導入していなかった |
| `uv run pytest tests/unit/test_ci.py -q -p no:cacheprovider` | passed (3 passed) | green: Linux jobだけが`libegl1`を導入し、3 OS offscreen環境の契約を確認した。修正後のremote matrixはPRで再確認する |
| PR #18 のCI | passed (6 jobs) | `libegl1`導入後、Ubuntu / macOS / WindowsのPython 3.12 / 3.13 matrixがすべてpassedした |
| `uv run python -c "<wheel / sdist metadata and file assertions>"` | expected failed (AssertionError) | red: PySide6導入前の既存wheelには`Requires-Dist: PySide6>=6.11,<6.12`がなく、Qt shell moduleも含まれていなかった |
| `uv build` | passed | source distributionとwheelを再生成した |
| `uv run python -c "<wheel / sdist metadata and file assertions>"` | passed | wheelのPySide6 requirementと`demi/ui/application.py`、`demi/ui/main_window.py`、sdistの対応する`src/demi/ui/` pathを確認した |
| `uv sync --dev` | passed | 77 packagesを解決し、74 packagesを確認した |
| `uv run ruff format --check .` | passed | 81 filesが整形済みだった |
| `uv run ruff check .` | passed | lint errorなし |
| `uv run ty check --no-progress` | passed | current source treeの型検査を実行した |
| `uv run pytest tests/unit` | passed (167 passed) | Qt shell、legacy pyglet境界、Ubuntu EGL workflow契約を含む機材不要suiteがgreen |
| `uv run pytest tests/integration` | passed (14 passed) | Qt lifecycleを含むintegration suiteがgreen |
| `uv build` | passed | final gateでsource distributionとwheelを再生成し、PySide6 metadataとQt shell moduleを再検査した |
| `uv run demi --version` / `uv run project-demi --version` / `uv run python -m demi --version` | passed | 3つのdisplay-free entry pointがいずれも`0.1.0`を出力した |
| `rg -n "TODO|TBD|xxx|前回|今回|一旦|上述|適宜|必要に応じて" README.md spec/complete/unit_014 .github/workflows/ci.yml` | passed | 該当なし。READMEの最小Qt画面、未実装機能、単体配布物未提供の文言を確認した |
...
- [x] QApplication / QMainWindow / event loopを最小構成で接続した
- [x] window stateとordered closeを接続した
- [x] import / versionのdisplay-free契約を維持した
- [x] offscreen fixtureと3 OS dependency testを追加した
- [x] `uv lock --check`と`uv build`を含むgateを記録した
- [x] TDD Test Listを更新した
- [x] unit_015への引き渡し条件を満たした
