...
| 例外を分類する | discovery、open、reconnect、pairing、apply、close の swbt exception | `ControllerAdapterError` 経由で runtime の category へ変換する | UI へ traceback や bond 内容を渡さない |
...
| refactor-done | 全 LogicalButton、normalized stick、rad/s gyro、G accel が public swbt values へ変換される | new / edge | unit | 3 tests green。raw 換算を行わず、実物の swbt value validation を使用 |
...
| refactor-done | AdapterInfo と ControllerColors が安全な Project_Demi value へ変換される | new / edge | unit | 1 test green。bond 内容、秘密値、USB handle を返さない |
| refactor-done | saved reconnect と explicit pairing が open / connect の順序と引数を守る | new / integration | integration | 1 test green。fake gamepad factory で `allow_pairing` と bond path を記録 |
| refactor-done | frame apply、disconnect、recreate、close が controller lifecycle を壊さない | new / regression | integration | 2 tests green。current reference の破棄、saved timeout 維持、neutral close、冪等 close を確認 |
| refactor-done | swbt exception が ControllerErrorCategory へ分類される | new / edge | unit | 5 tests green。adapter と runtime event の discovery、bond、timeout、input、transport を確認し、swbt type は境界外へ漏らさない |
...
| refactor-done | unit 全 gate、integration、build、wheel smoke が通る | characterization | package | 109 unit、6 integration、lock、format、lint、ty、build、wheel smoke が pass。hardware は対象外として記録 |
...
- error category の mapping は swbt class 名を Project_Demi の public event に出さず、`ControllerAdapterError` に閉じ込める。
...
| `uv run pytest tests/unit/controller/test_swbt_adapter.py -q` | passed | 3 passed。frame conversion、neutral、AdapterInfo、ControllerColors を確認 |
...
| `uv run pytest tests/unit/controller/test_runtime.py -q` | passed | 2 passed。worker cleanup と adapter error category の runtime event を確認 |
| `uv run pytest tests/integration/controller/test_swbt_lifecycle.py -q` | passed | 2 passed。saved reconnect、pairing、frame apply、色再生成、close を確認 |
| `uv run pytest tests/unit/controller/test_swbt_errors.py -q` | passed | 4 passed。discovery、InvalidKeyStore、pairing timeout、transport open、invalid input の adapter category を確認 |
| `uv lock --check` | passed | Resolved 68 packages |
| `uv run ruff format --check .` | passed | 66 files already formatted |
| `uv run ruff check .` | passed | All checks passed |
| `uv run ty check --no-progress` | passed | All checks passed |
| `uv run pytest tests/unit` | passed | 109 passed |
| `uv run pytest tests/integration` | passed | 6 passed |
| `uv build` | passed | `dist/demi_controller-0.1.0.tar.gz` と `dist/demi_controller-0.1.0-py3-none-any.whl` を生成 |
| `uv run python -c "from pathlib import Path; import zipfile; wheel=next(Path('dist').glob('*.whl')); archive=zipfile.ZipFile(wheel); names=set(archive.namelist()); metadata=archive.read(next(name for name in names if name.endswith('/METADATA'))).decode(); required={'demi/controller/swbt_adapter.py','demi/controller/runtime.py'}; assert required <= names, sorted(required-names); assert 'Requires-Dist: swbt-python' in metadata; print(f'{wheel.name}: adapter files and swbt-python metadata present')"` | passed | wheel に adapter files と `swbt-python` dependency metadata が含まれることを確認 |
