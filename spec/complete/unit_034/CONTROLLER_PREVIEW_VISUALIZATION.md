...
| refactor-skipped | accelの正負3軸がvector方向と長さへ変換され、上限を超える値でboundsを越えない | new / edge | unit | 8件green。gyroと共通のsigned axis modelを再利用し追加の構造変更なし |
| refactor-skipped | pointer captureとkeyboard operational stateが独立した表示値としてpreviewへ渡る | new / regression | unit / integration | unit_033の既存実装がgreen。独立した2値をそのまま渡すため追加の構造変更なし |
| refactor-skipped | tooltipまたはaccessible descriptionからgyro / accelの軸名と数値を取得できる | new | integration | 5件green。同一フレームから共通説明文を生成するため追加の構造変更なし |
| refactor-done | relative layoutとsigned sensor modelが実際のQPainter描画へ使われ、常時表示の数値列が残らない | new / regression | unit / integration | 19件green。旧絶対座標描画と数値列を相対control・signed sensor描画へ置換 |
| refactor-done | Windows通常描画のneutral、全button代表、sensor正負、resize画面で重なりと不自然な余白がない | new | manual | minimumの状態文切れと境界重なりを修正後、英語・日本語を再撮影して解消を確認 |
...
| `uv run pytest -p no:cacheprovider --basetemp tmp/pytest/unit034-paint-green tests/unit/ui/test_controller_preview.py tests/unit/ui/test_preview_layout.py tests/unit/ui/test_preview_sensor.py tests/integration/ui/test_controller_preview_accessibility.py -q` | pass | 19 passed。relative layout、signed sensor model、補助情報を同一frameから描画境界へ渡すことを確認 |
| `PYTHONUTF8=1 uv run pytest -p no:cacheprovider --basetemp tmp/pytest/unit034-catalog-network tests/integration/package/test_translation_catalog.py -q` | pass | 1 passed。sandbox内ではbuild依存取得が拒否されたため、許可済みnetworkで再実行 |
| `.venv/Scripts/pyside6-lrelease.exe src/demi/i18n/demi_ja.ts -qm src/demi/i18n/demi_ja.qm` | pass | 115件finished、unfinished 0件 |
| `$inspect-gui-states` Windows通常描画 | pass | neutral/default、全入力とsensor正負、minimum、largeを確認。minimumの状態文切れと本体境界重なりを修正し、英語・日本語で再撮影して解消を確認 |
...
| `uv run pytest -p no:cacheprovider --basetemp tmp/pytest/unit034-accel tests/unit/ui/test_preview_sensor.py -q` | pass | 8 passed。accel正負3軸の方向、vector長、上限clampを確認 |
| `uv run pytest -p no:cacheprovider --basetemp tmp/pytest/unit034-status tests/unit/ui/test_controller_preview.py -q` | pass | keyboard operational=true、pointer capture=falseを独立した表示値として保持することを確認 |
| `uv run pytest -p no:cacheprovider --basetemp tmp/pytest/unit034-access-green tests/integration/ui/test_controller_preview_accessibility.py tests/unit/ui/test_controller_preview.py -q` | pass | 5 passed。gyro / accelの3軸値と単位をtooltip、accessible descriptionから取得できることを確認 |
| `uv run ruff check src/demi/ui/controller_preview.py tests/integration/ui/test_controller_preview_accessibility.py` | pass | 指摘なし |
| `uv run ty check --no-progress` | pass | 型エラーなし |
| `uv run pytest -p no:cacheprovider --basetemp tmp/pytest/unit034-visual-final ... -q` | pass | 22 passed。preview unit、accessibility integration、文書構造をまとめて確認 |
| `uv run ruff format --check .` / `uv run ruff check .` | pass | 147 files formatted、lint指摘なし |
| `uv run ty check --no-progress` | pass | 公開preview modelを含め型エラーなし |
| `uv run pytest -p no:cacheprovider --basetemp tmp/pytest/unit034-unit tests/unit` | pass | 273 passed |
| `uv run pytest -p no:cacheprovider --basetemp tmp/pytest/unit034-integration-network tests/integration --ignore=tests/integration/ui` | pass | 15 passed。package smokeの依存取得がsandboxで拒否されたため許可済みnetworkで再実行 |
| `uv run pytest -p no:cacheprovider --basetemp tmp/pytest/unit034-ui-main2 tests/integration/ui --ignore=tests/integration/ui/test_source_entry_points.py` | pass | 69 passed |
| `uv run pytest -p no:cacheprovider --basetemp tmp/pytest/unit034-ui-entry2 tests/integration/ui/test_source_entry_points.py` | pass | 3 passed。Qt top-level teardownの既知abortを避けるためentry point群を分割 |
| `uv sync --dev` / `uv lock --check` | pass | 77 packages resolved、74 packages checked |
...
- 実機からのgyro / accel入力による図形の妥当性確認はnot run。unit_034では合成`ControllerFrame`による符号、clamp、通常描画を確認し、実機確認はhardware acceptanceを扱う別unitへ送る。
...
- [x] 全control inventoryとgeometry testをgreenにした
- [x] gyro / accel表示を正負の合成frameで確認した
