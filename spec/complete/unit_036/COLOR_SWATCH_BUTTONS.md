...
| refactor-skipped | 英語と日本語でrow labelとaccessible textが切り替わり、hex値自体は変わらない | new / regression | integration | 26件green。既存translation境界を使いfield labelと操作文言だけを翻訳 |
| refactor-skipped | Windows通常描画で各swatch、keyboard focus、light / dark fillの識別が不自然でない | new | manual | 4 PNGを原寸確認。白、黒、背景同色に輪郭があり、白・黒focusは太いhighlight枠で判別可能 |
...
| `spec/complete/unit_036/COLOR_SWATCH_BUTTONS.md` | new | 完了した作業境界と検証記録 |
...
| `uv run pytest -p no:cacheprovider --basetemp tmp/pytest/unit036-localization tests/integration/ui/test_localization.py tests/integration/ui/test_colors_dialog.py -q` | pass | 26 passed。英語・日本語のrow label、accessible name / description、非翻訳hex値を確認 |
| `uv run python .agents/skills/inspect-gui-states/scripts/capture_gui.py --scenario tmp/gui-audit/unit-036-swatches/scenario.py --output tmp/gui-audit/unit-036-swatches` | pass | Windows通常描画で既定、白・黒・背景同色、白focus、黒focusの4 PNGを取得。輪郭、focus、文字配置を原寸確認し追加修正なし |
| `uv sync --dev` / `uv lock --check` | pass | 77 packagesを解決しlock整合を確認 |
| `uv run ruff format --check .` / `uv run ruff check .` / `uv run ty check --no-progress` | pass | 148 filesのformat、lint、型境界を確認 |
| `uv run pytest -p no:cacheprovider --basetemp tmp/pytest/unit036-unit-sequential tests/unit` | pass | 278 passed。並列実行時のbasetemp競合後、単独で再実行 |
| `uv run pytest -p no:cacheprovider --basetemp tmp/pytest/unit036-int-nonui-seq tests/integration --ignore=tests/integration/ui` | pass | 15 passed |
| `uv run pytest -p no:cacheprovider --basetemp tmp/pytest/unit036-int-ui-seq tests/integration/ui --ignore=tests/integration/ui/test_source_entry_points.py` | pass | 100 passed |
| `uv run pytest -p no:cacheprovider --basetemp tmp/pytest/unit036-entry-seq tests/integration/ui/test_source_entry_points.py` | pass | 3 passed |
...
- [x] swatchのmouse / keyboard操作をgreenにした
- [x] Save / Cancel / reconnectの回帰をgreenにした
