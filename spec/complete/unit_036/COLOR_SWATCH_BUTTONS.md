...
| `uv run pytest -p no:cacheprovider --basetemp tmp/pytest/unit036-unit tests/unit -q` | pass | 278 passed |
| `PYTHONUTF8=1 uv run pytest -p no:cacheprovider --basetemp tmp/pytest/unit036-integration-nonui-utf8 tests/integration --ignore=tests/integration/ui -q` | pass | 15 passed。package build / installを含むためnetwork許可下で実行 |
| `uv run pytest -p no:cacheprovider --basetemp tmp/pytest/unit036-ui-main tests/integration/ui --ignore=tests/integration/ui/test_source_entry_points.py -q` | pass | 100 passed |
| `uv run pytest -p no:cacheprovider --basetemp tmp/pytest/unit036-ui-entry tests/integration/ui/test_source_entry_points.py -q` | pass | 3 passed |
