...
| initial design | milestone 0でPySide6 / Qt Widgets前提へ更新済みの現行設計 | `spec/initial/` |
| completed history | 撤去対象を導入・配線・配布した履歴 | `spec/complete/unit_004/`, `spec/complete/unit_010/`, `spec/complete/unit_011/` |
| current source | 旧UI package / input adapterを削除し、applicationに残すのはtoolkit非依存のsession境界だけ | `src/demi/app.py`, `src/demi/application/` |
| current package | pyglet dependency、収集指定、license inventory、旧release workflowを削除済み | `pyproject.toml`, `uv.lock`, `packaging/`, `.github/workflows/` |

milestone 0は本unitの実装着手前に完了し、`spec/initial`のPySide6 / Qt Widgets前提を実装判断の正本として使用した。完了済み作業仕様は当時の実装記録として保持し、現行仕様へ合わせて書き換えない。
...
| UI 以外を維持する | domain、config、mapping、controller の対象試験 | pyglet 撤去を理由に失敗しない | full unit / integration gate は green。Qt UI の復旧は後続 unit の対象 |
...
| refactor-skipped | wheel と sdist が削除済み `demi.ui` と `demi.input.pyglet_backend` を含まず、pyglet を依存に持たない | regression | package | `uv build`で再生成し、archive contentsとmetadataを直接検査 |
| refactor-skipped | domain、config、input mapping、controller runtime の機材不要対象試験が pyglet なしで成功する | characterization | unit | `uv sync --dev`がpygletをuninstallした後にunit suite 156件がgreen。Qt UI復旧はunit_014以降で扱う |

## 7. 設計メモ

### 7.1 確認済みの事実

- `pyproject.toml`、`uv.lock`、開発環境からpygletを削除した。`uv sync --dev`はpyglet 2.1.15をuninstallし、`uv lock --check`は73 packagesで成功した。
- `src/demi/ui`、`src/demi/input/pyglet_backend.py`、旧UI専用testを削除した。現行source / testに旧importまたは具象型参照がないことをAST testと検索で確認した。
- `src/demi/app.py`から旧UI factoryと具象型を外し、`ApplicationSession`、`CaptureCoordinator`、`InputPublisher`、`ApplicationShutdownCoordinator`のtoolkit非依存契約を維持した。
- 旧`package.yml`を削除し、PyInstaller builderとlicense inventoryからpygletを削除した。Qt standalone workflowはmilestone 7まで作成しない。
...
- GUI不在はunit_014までの明示的な中間状態であり、このunitの完了をGUI機能またはrelease readinessの達成と扱わない。

### 7.3 unit 間の引き渡し

- 着手条件: milestone 0により`spec/initial`の採用UI、input、lifecycle、testing、risk、diagnosticsをPySide6前提へ更新済みである。
- unit_014へ引き渡した条件: pygletのproduction / package境界が消え、package import、3 version entry point、lock、source package buildが成立し、GUI不在が安全に分類されている。
...
| `spec/complete/unit_013/LEGACY_UI_AND_PYGLET_REMOVAL.md` | modify / move | TDD 状態、検証、引き渡し記録 |

## 9. 検証

| command | result | notes |
|---|---|---|
| `rg -n "T[O]DO|T[B]D|x[x]x|前[回]|今[回]|一[旦]|上[述]|適[宜]|必要に応じ[て]" spec/complete/unit_013` | passed | 該当なし |
| `git diff --no-index --check -- NUL spec/complete/unit_013/LEGACY_UI_AND_PYGLET_REMOVAL.md` | passed | whitespace errorなし。LF / CRLF変換予告のみ |
...
| `tar -tf dist\demi_controller-0.1.0-py3-none-any.whl` | passed | wheelに`demi/ui`と`demi/input/pyglet_backend.py`がない |
| `tar -tf dist\demi_controller-0.1.0.tar.gz` | passed | sdistに旧UI source moduleがない。spec内の履歴記録は判定対象外 |
| `tar -xOf <artifact> .../METADATA` / `PKG-INFO` | passed | wheel / sdist metadataにpyglet requirementがない |
| `uv sync --dev` | passed | 73 packagesを解決し、pyglet 2.1.15を開発環境からuninstallした |
| `uv run pytest tests/unit -q` | passed (156 passed) | pyglet非導入環境でdomain / config / mapping / controllerを含む機材不要suiteがgreen |
...
| `uv run ruff format --check tests/unit/test_packaging.py` | passed | formatter差分なし |
| `uv run ruff check tests/unit/test_packaging.py` | passed | lint指摘なし |
| `uv sync --dev` | passed | 73 packagesを解決し、pyglet 2.1.15をuninstallした |
| `uv lock --check` | passed | 73 packagesでlockfile整合を確認した |
| `uv run ruff format --check .` | passed (75 files) | formatter差分なし |
| `uv run ruff check .` | passed | lint指摘なし |
| `uv run ty check --no-progress` | passed | type error / warningなし |
| `uv run pytest tests/unit -q` | passed (156 passed) | pyglet非導入環境の機材不要suiteがgreen |
| `uv run pytest tests/integration -q` | passed (12 passed) | controller runtime integrationがgreen |
| `uv run demi --version` / `uv run project-demi --version` / `uv run python -m demi --version` | passed | すべて`0.1.0`、status 0 |
| `uv run python -c "import demi; print(demi.__version__)"` | passed | `0.1.0`、display / GUI runnerを生成しない |
| `uv build` | passed | source distributionとwheelを再生成し、archive contents / metadata smokeもgreen |
| `git diff --check` | passed | whitespace errorなし |

full unit / integration gateはgreenである。引数なしGUI起動、Qt dependency、Qt実display、PySide6 standaloneは対象外としてunit_014以降またはmilestone 7へ明示的に引き渡す。
...
- [x] milestone 0 の初期仕様更新が完了している
- [x] `src/demi/ui` と `pyglet_backend.py` を一括削除した
- [x] app、test、type、factory から pyglet 具象依存を削除した
- [x] metadata、lock、builder、license、pytest marker から pyglet を削除した
- [x] 旧 standalone release artifact を停止した
- [x] package import、3 version entry point、non-UI対象試験を確認した
- [x] TDD Test List を更新した
- [x] 検証結果と未実行理由を記録した
- [x] unit_014 への引き渡し条件を満たした
