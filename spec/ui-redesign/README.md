...
初期設計の正本は引き続き `spec/initial/` とし、UI 再設計の目標状態は milestone 0 で
反映済みである。このディレクトリは移行判断と作業分割の根拠を記録する。記述が
`spec/initial/` と矛盾した場合は、実装へ進む前に同じ変更内で解消する。各マイルストーンの
作業仕様は `spec/wip/unit_XXX` に置く。
...
- 現行 runtime dependency は `pyglet>=2.1,<2.2` であり、milestone 1 の撤去対象である。
