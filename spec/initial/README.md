...
| UI | コントローラー可視化、ツールバー、設定用モーダル |
| 実行モデル | pyglet主スレッド + asyncio接続ワーカー |
...
pygletのイベントループは主スレッドに置く。`swbt-python` は専用スレッド上の `asyncio` イベントループで動かす。両者はコマンドと不変イベントだけで通信する。
...
`pyglet.window.key`、`pyglet.window.mouse`、`swbt.Button`、`swbt.InputState` は、それぞれのアダプター内でだけ使う。設定、試験、入力変換の中心はProject_Demi独自の型で表現する。
...
OS keyboard / mouse events
            │
            ▼
PygletInputBackend ──► PhysicalInputState
                              │
                              ▼
                        InputMapper
                              │
...
| 外部事実 | pygletはキー押下・解放、マウスボタン、相対マウス移動、排他マウスモードを提供する |
...
| 未検証 | pygletの相対マウス入力が全OS・全マウスで同じ加速特性になる保証はない |
