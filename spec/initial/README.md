...
| 起動確認範囲 | ソース一式とビルドした wheel で、`demi`、`project-demi`、`python -m demi` が同じQt GUIを起動する |
| 単体配布 | Windows用の単体配布物は提供せず、OS別パッケージングは将来作業とする |
...
キーイベントごとに `press()` や `release()` を直接呼ばない。各時点の保持入力から完全な
`ControllerFrame`を生成し、接続境界で`swbt.InputState`へ変換して、Bumbleの送信キューへ
受理されるまで`send()`を待つ。
