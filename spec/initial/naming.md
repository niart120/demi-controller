...
`Manager` と `Handler` は責務が曖昧になりやすいため原則使わない。pygletのイベントハンドラーとして意味が限定される場合だけ `Handler` を認める。
...
```text
feat(input): add pyglet relative mouse backend
