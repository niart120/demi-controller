...
合成pitchには入力元によらず同じ設定上限を適用し、上限で外向きの入力を受けた場合は実際に適用した角変位とジャイロYをともに0にする。Y 軸キーの解放後は解放時の姿勢を維持する。Z 軸診断はワールド上方向まわりのyawとして区間中央pitchでジャイロX/Zへ投影し、静的加速度は変更しない。捕捉解除、捕捉epoch変更、再設定では単一のpitchを0へ戻す。
