...
`Esc` は待受だけを中止し、draftを変えない。Esc自体は対象行を選び「Escを割り当てる」を実行する。`F4` は予約理由を対象行へ表示して待受を継続し、`F12` は通常sourceとして確定する。別行で使用中のsourceは即時変更せず、既存targetと変更先を示す置換確認を通す。割り当て領域からマウスジャイロ設定tabへ移動した場合は、隠れた待受を残さず中止する。
