...
`Ctrl+C` はマウス捕捉切替として使える。文字入力欄や設定画面では通常の文字操作を妨げない。`F4` は常にマウス捕捉解除を優先し、ユーザー割り当て不可とする。`F12`は通常の割り当てへ使用できる。
...
`ACCEL:ZERO` のsourceを保持している間は、`RotationPoseModel` のpitchを更新したまま、最終 `ControllerFrame.accel_g` だけを完全な0Gへ置換する。解放後はpitchを初期化せず、現在姿勢に対応する静的1Gへ戻す。これは対象側の挙動を比較する診断例外であり、通常の静止状態には使わない。
