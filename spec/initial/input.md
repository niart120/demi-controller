...
Y 軸の診断角速度は実経過時間でキー由来の仮想pitchへ積分する。静的加速度は、設定上限内のマウス由来pitchとキー由来pitchを合成した姿勢から `(-sin(pitch), 0, cos(pitch)) G` として生成する。キー由来pitchは固定角速度との整合を保つため、マウス操作用のpitch上限を適用しない。Y 軸キーの解放後は解放時の姿勢を維持する。Z 軸診断はワールド上方向まわりのyawなので静的加速度を変更しない。捕捉解除、捕捉epoch変更、再設定では両方のpitchを0へ戻す。
