...
- マウス捕捉外ではmouse sourceが常にニュートラルで、focused operational stateのkeyboard sourceは評価する
...
- フォーカス喪失、設定画面、shutdownで診断target由来の姿勢を水平へ戻し、マウス捕捉epoch変更ではkeyboard由来姿勢を維持する
- `IMU:NEUTRAL` 保持中は最終フレームを物理ニュートラルIMUにし、診断回転より優先する
- キー解放と全入力中立化で診断入力が残留せず、マウス捕捉解除ではkeyboard保持を維持する
...
- ジャイロの横棒が円形操作部品と異なる縦横比を持ち、中央から左右へ符号と単調な表示量を示すこと
- 加速度の3成分が1本のベクトルへ合成され、+X＝上、+Y＝左下、+Z＝右下の補助軸方向と一致し、表示量が単調かつ上限でclampされること
...
- 4つのtabが1つのdraftとSave / Cancelを共有する
- Connection tabで全体設定を上、接続プロファイル操作を下に区別し、Saveでは接続commandを発行しない
