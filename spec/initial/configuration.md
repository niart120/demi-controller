...
I/J/K/L の固定ジャイロ診断入力は設定へ保存せず、profile binding より優先する。`I / K` は Y 軸へ `-1.0 / +1.0 rad/s`、`J / L` は Z 軸へ `+1.0 / -1.0 rad/s` を入力する。保存済み profile に同じキーの binding があっても、入力捕捉中は固定診断入力だけを評価する。
