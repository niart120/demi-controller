...
アプリ用の翻訳元 `.ts` と配布・実行時に読み込む `.qm` は同期する。主要な状態表示は `.qm` の実際の翻訳結果を検査し、catalogが読み込めることだけを同期の根拠にしない。
...
- 本体: 肩側から中央へ浅く下がる連続曲線と、上側面から左右グリップへつながる独自の簡略シルエット。中央の下がりは本体高さの10%以内とし、ショルダーとグリップを含む外接矩形の幅と高さの比を1.45:1から1.60:1にする。左右グリップの外側境界は本体の左右境界内に収め、下側の丸みだけを残す
- フェイスプレート上の操作部: 操作部全体をフェイスプレート内に収め、外形線を操作部の背後へ描かない
- スティック: 外周円と可動ノブ
- A/B/X/Y: 円。上下と左右の中心間隔を等しくする
- 十字キー: 内枠のない単一の十字パス。押下方向の腕だけを押下色で塗り分け、全幅と全高を左スティック直径の120%以内にする
- L/R/ZL/ZR: 連続したフェイスプレート上段へ同じ高さで収める丸角矩形。幅はフェイスボタン直径の225%以内にする
- Plus/Minus/Home/Capture: ABXY直径の80%以下に揃えた円。MinusとPlusを上側外寄り、Captureを下側左、Homeを下側右へ置き、左右対称の浅いV字にする
- 左スティック、十字キー、小型操作部、ABXY: 左スティックとABXYをショルダー段の直下、十字キーを左スティックの右下に置く。十字キーと右スティックの中点は画面中央から描画領域幅の2%以内に収める。左スティックと小型操作部、小型操作部とABXYの横方向の空白をそれぞれ描画領域幅の7%以内にする
...
- ボタン集合: 押下オーバーレイ
- スティック: 入力方向を保って中心から移動し、最大斜め入力でも可動ノブ全体を外周円内へ収める
- ジャイロ: 3軸を中央が0の横棒として並べ、負を左、正を右、角速度の大きさを棒の長さで示して表示用上限へclampする。操作ボタンと同じ円形を使わない。rad/sの値はtooltipとアクセシビリティ説明へ置く
- 加速度: 薄い3軸補助線と `+X` / `+Y` / `+Z` ラベルを表示し、3成分を模式投影した1本の矢印で符号と大きさを示して表示用上限へclampする。右Joy-Conの[軸定義](https://github.com/dekuNukem/Nintendo_Switch_Reverse_Engineering/blob/master/imu_sensor_notes.md#axes-definition)に合わせ、画面上では +Xを右上、+Yを右下、+Zを下へ投影する。G単位の値はtooltipとアクセシビリティ説明へ置く
- マウス入力状態: コントローラー図の下、IMU表示の上へON/OFFを表示する。F5のショートカット表記は含めず、状態を色だけに依存せず文字でも判別できる
- 横方向の整列: コントローラー本体、マウス入力状態、IMU全体の左右端を揃える。GyroとAccelerationは同じ幅とし、中央に描画領域幅の4%の間隔を置く
- 切断: 半透明の「未接続」表示。ただしプレビューは継続

800x520のウィンドウ内にある800x475のプレビュー領域では、ジャイロと加速度へそれぞれ90px以上の高さを確保し、見出し帯とグラフ領域を分ける。加速度の軸ラベルは10pxを下限とする。
