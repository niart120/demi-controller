...
- IMU: ジャイロ3軸の符号付き円弧と、共通原点から伸びる加速度3軸ベクトル
...
- ボタン集合: 押下オーバーレイ
- スティック: 中心から半径内へ移動
- ジャイロ: 3軸の符号を円弧の向き、大きさを円弧長で示し、表示用上限へclampする。rad/sの値はtooltipとアクセシビリティ説明へ置く
- 加速度: 3軸の符号と大きさを共通原点からのベクトルで示し、表示用上限へclampする。G単位の値はtooltipとアクセシビリティ説明へ置く
