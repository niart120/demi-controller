...
- 待受中は表領域で押した対応マウスボタンを候補にする。ただし反転、変更、削除の操作セルは候補にせず、通常の表操作を優先する。
- 競合がある場合はsource、変更先、既存targetを列挙し、置換、両方を保持、取消を選ばせる。取消時はdraftを変更しない。両方を保持は確認ダイアログで利用者が明示的に選んだ場合だけ適用する。
