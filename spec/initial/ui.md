...
各actionは同じ設定ダイアログを開き、選択した設定tabを前面にする。
...
- 入力状態: キーボード入力可能状態とマウス捕捉状態を別の値として示す
- 切断: 半透明の「未接続」表示。ただしプレビューは継続

## 5. 設定ダイアログ

`QDialog`、`QTabWidget`、共通の `QDialogButtonBox` を使い、`Mappings`、`Connection`、`Colors` を切り替える。3つのtabは1つのdraftを共有し、Saveは全tabの変更を1回で保存する。CancelとEscは全tabの変更を破棄し、色previewを保存値へ戻す。
...
一覧は `QTableView` とmodelで表示し、Qt標準のスクロールを使う。割り当てとマウスジャイロ設定はtabで分け、全項目を1画面に詰め込まない。canonical sourceはmodel roleへ保持し、表には利用者向けの入力名を表示する。
...
- マウスジャイロtabへ移動した場合は待受を中止し、非表示の待受状態を残さない。
- targetを選んで未割り当て行を末尾へ追加し、選択行だけを削除できる。
